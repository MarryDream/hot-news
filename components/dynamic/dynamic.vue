...
const state = reactive( {
	type: props.type,
	rich_text_nodes: props.dynamic.desc?.rich_text_nodes,
	majorItems: [ {
		url: "",
		style: {}
	} ],
...
switch ( props.type ) {
	case 'DYNAMIC_TYPE_DRAW':
		...
		let drawItems: Opus_Pic[] | DrawItem[];
		if ( maj!.type === 'MAJOR_TYPE_OPUS' ) {
			drawItems = ( <BiliDynamicMajorOpus>maj ).opus.pics;
			state.rich_text_nodes = ( <BiliDynamicMajorOpus>maj ).opus.summary.rich_text_nodes;
		} else {
			drawItems = ( <BiliDynamicMajorDraw>maj ).draw.items;
		}
	...
	case 'DYNAMIC_TYPE_AV':
		state.majorArchive.cover = `${ ( <BiliDynamicMajorArchive>props.dynamic.major ).archive.cover }@472w_264h_1c.webp`
		break;

	case 'DYNAMIC_TYPE_ARTICLE':
		const covers = ( <BiliDynamicMajorArticle>props.dynamic.major ).article.covers;
		const cover = covers && covers.length > 0 ? covers[0] : "";
		state.majorArticle.cover = `${ cover }@2072w.webp`;
		break;
	case 'DYNAMIC_TYPE_LIVE_RCMD':
		const live = JSON.parse( ( <BiliDynamicMajorLive>props.dynamic.major ).live_rcmd.content );
		state.live = {
	...
	case 'DYNAMIC_TYPE_LIVE':
		const major = <BiliDynamicMajorLiveShare>props.dynamic.major;
		state.live = {
			title: major.live.title,
			link: "https:" + major.live.jump_url,
			live_status: major.live.badge.text,
			area_name: major.live.desc_first,
			watched_show: major.live.desc_second,
			cover: {
				avif: `${ major.live.cover }@472w_264h_1c_!web-dynamic.avif`,
				webp: `${ major.live.cover }@472w_264h_1c_!web-dynamic.webp`
			}
		};
		break;
}
...
<template>
	<div class="dynamic-main">
		<div class="opus-module-content">
			<p>
				<div v-if="dynamic.topic" class="bili-dyn-content__orig__topic">
					<div class="bili-dyn-topic">
						<div class="bili-dyn-topic__card">
							<svg class="bili-dyn-topic__icon" viewBox="0 0 16 16">
				...
				<template v-for="item in state.rich_text_nodes">
					<span v-if="item.type === 'RICH_TEXT_NODE_TYPE_LOTTERY'"
					      class="opus-text-rich-hl lottery">{{ item.text }}</span>
					<a v-if="item.type === 'RICH_TEXT_NODE_TYPE_TOPIC'" :href="item.jump_url"
					   class="opus-text-rich-hl topic"
					   target="_blank">{{ item.text }}</a>
					<span v-if="item.type === 'RICH_TEXT_NODE_TYPE_TEXT'" style="font-size:17px;">{{ item.text }}</span>
					<span v-if="item.type === 'RICH_TEXT_NODE_TYPE_AT'"
					      class="opus-text-rich-hl at">{{ item.text }}</span>
					<a v-if="item.type === 'RICH_TEXT_NODE_TYPE_WEB'" :href="(item as RichTextNodeWeb).jump_url"
					   class="opus-text-rich-hl link"
					   target="_blank">{{ item.text }}</a>
					<span v-if="item.type === 'RICH_TEXT_NODE_TYPE_VOTE'"
					      class="opus-text-rich-hl vote">{{ item.text }}</span>
					<span v-if="item.type === 'RICH_TEXT_NODE_TYPE_BV'"
					      class="opus-text-rich-hl video">{{ item.text }}</span>
					<span v-if="item.type === 'RICH_TEXT_NODE_TYPE_AV'"
					      class="opus-text-rich-hl video">{{ item.text }}</span>
					<span v-if="item.type === 'RICH_TEXT_NODE_TYPE_OGV_SEASON'"
					      class="opus-text-rich-hl video">{{ item.text }}</span>
					<img v-if="item.type === 'RICH_TEXT_NODE_TYPE_EMOJI'" :alt="item.text"
					     :src="(item as RichTextNodeEmoji).emoji.icon_url"
					     class="opus-text-rich-emoji">
					<img v-if="item.type === 'RICH_TEXT_NODE_TYPE_GOODS'" :alt="item.text"
			...
			<div v-if="type === 'DYNAMIC_TYPE_DRAW'" class="opus-para-pic">
				<div class="bili-album">
					<div v-if="state.majorClass === 'one'" class="bili-album__watch">
						<div class="bili-album__watch__content">
							<img :src="state.majorItems[0].url" alt="动态图片">
						</div>
					</div>
					<div v-else :class="state.majorClass" class="bili-album__preview">
						<div v-for="item in state.majorItems" :style="item.style" class="bili-album__preview__picture">
							<img :src="item.url" alt="动态图片"
							     class="bili-album__preview__picture__img bili-awesome-img"/>
						</div>
					</div>
				</div>
			</div>

			<div v-if="type === 'DYNAMIC_TYPE_AV'" class="bili-dyn-content__orig__major suit-video-card gap">
				<a :href="(dynamic.major as BiliDynamicMajorArchive).archive.jump_url"
				   class="bili-dyn-card-video hide-border">
					<div class="bili-dyn-card-video__header">
						<div class="bili-dyn-card-video__cover">
							<img :src="state.majorArchive.cover" alt="视频封面" class="bili-awesome-img"/>
							...
							<div class="bili-dyn-card-video__cover-shadow">
								<div class="duration-time">
									{{ ( dynamic.major as BiliDynamicMajorArchive ).archive.duration_text }}
								</div>
							</div>
						</div>
					</div>
					<div class="bili-dyn-card-video__body">
						<div class="bili-dyn-card-video__title bili-ellipsis fs-medium">
							{{ ( dynamic.major as BiliDynamicMajorArchive ).archive.title }}
						</div>
						<div class="bili-dyn-card-video__desc bili-ellipsis fs-small">
							{{ ( dynamic.major as BiliDynamicMajorArchive ).archive.desc }}
						</div>
						<div class="bili-dyn-card-video__stat fs-small">
							<div class="bili-dyn-card-video__stat__item play"><i></i>
								<span>{{ ( dynamic.major as BiliDynamicMajorArchive ).archive.stat.play }}</span></div>
							<div class="bili-dyn-card-video__stat__item danmaku"><i></i>
								<span>{{ ( dynamic.major as BiliDynamicMajorArchive ).archive.stat.danmaku }}</span>
							</div>
						</div>
					</div>
				</a>
			...
			<div v-if="type === 'DYNAMIC_TYPE_ARTICLE'" class="bili-dyn-content__orig__major suit-video-card">
				<div class="dyn-card-opus hide-border">
					<div class="dyn-card-opus__title bili-ellipsis">
						{{ ( dynamic.major as BiliDynamicMajorArticle ).article.title }}
					</div>
					<div class="dyn-card-opus__summary">
						<div class="bili-rich-text" style="font-size: 15px;">
							<div class="bili-rich-text__content folded line--4"
							     style="line-height: 25px; max-height: 100px;">
								<span class="virturl-start"></span>
								<span>{{ ( dynamic.major as BiliDynamicMajorArticle ).article.desc }}</span>
							</div>
							<div class="bili-rich-text__action">全文</div>
						</div>
					</div>
					<div v-if="state.majorArticle.cover" class="bili-album__watch">
						<div class="bili-album__watch__content">
							<img :src="state.majorArticle.cover" alt="专栏封面">
						</div>
					</div>
				</div>
			</div>

			<div v-if="type === 'DYNAMIC_TYPE_LIVE_RCMD' || type === 'DYNAMIC_TYPE_LIVE'"
			     class="bili-dyn-content__orig__major suit-video-card">
				<a :href="state.live.link" class="bili-dyn-card-live rcmd hide-border">
					<div class="bili-dyn-card-live__header">
						<div class="bili-dyn-card-live__tag state--1">{{ state.live.live_status }}</div>
						<div class="bili-dyn-card-live__cover b-img">
							<picture class="b-img__inner">
								<source :srcset="state.live.cover.avif" type="image/avif">
								<source :srcset="state.live.cover.webp" type="image/webp">
								<img :src="state.live.cover.webp" alt="直播封面">
							</picture>
						</div>
					</div>
					<div class="bili-dyn-card-live__body">
						<div class="bili-dyn-card-live__title bili-ellipsis fs-medium">{{ state.live.title }}</div>
						<div class="bili-dyn-card-live__desc fs-small">{{ state.live.area_name }} ·
							{{ state.live.watched_show }}
						</div>
					</div>
				</a>
			</div>
