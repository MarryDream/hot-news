...
- 2023/03/01 处理结束直播后无法获取到直播时长的问题；直播动态不再处理，交由直播定时任务专门处理
- 2023/02/28 修复下载图片超时过长问题；修复开奖动态会被推送的问题；添加 `liveTime` 开播时长字段。
- 2022/12/10 更新60秒新闻图API。
- 2022/11/13 直播间信息获取优化，对于不存在的用户或者无直播间信息的情况进行处理。
- 2022/11/12 B站直播推送优化，直播结束时将同步删除缓存以便后续再次开播时还可以收到推送（原本的缓存时间仍保留以应对极端情况，可以自行根据使用情况设置更大的值）。
